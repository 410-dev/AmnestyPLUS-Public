...
@WebServlet(name = "UploadLetter", value = "/UploadLetter")
public class UploadLetter extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        int size = 1024 * 1024; // 저장가능한 파일 크기
        String file = ""; // 업로드 한 파일의 이름(이름이 변경될수 있다)
        String fileLocation = "/opt/tomcat/apache-tomcat-8.5.61/webapps/ROOT/files/taskforce/";
        // 실제로 파일 업로드하는 과정
        ...
        File regenerated = new File(fileLocation + newFileName + ".pdf");
        origFile.renameTo(regenerated);
        try {
            new LetterWrite().task(request, response, multi, newFileName + ".pdf");
        }catch (Exception e) {
            request.getSession().setAttribute("AMError", new PageExceptionPayload(e, "UploadLetter Servlet"));
